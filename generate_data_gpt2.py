...
def set_hooks_gpt2(model):
    ...
    def get_activation(name):
        ...
        # return hook
                # modification to handle experiment 2 to change # of layers tested and not be outside the range 
                if name == "layer_residual_" + str(final_layer) and "intermediate_residual_" + str(final_layer) in model.activations_ and "mlp_" + str(final_layer) in model.activations_:
                    model.activations_[name] = model.activations_["intermediate_residual_" + str(final_layer)] + model.activations_["mlp_" + str(final_layer)]
                
                elif name != "layer_residual_" + str(final_layer):
                    model.activations_[name] = input[0][:, num_tokens - 1].detach()
    ...
    # capturing last 3 (or last n layers) - 
    # for i in range(model.config.n_layer - 3, model.config.n_layer):

    # capturing only first 3 layers 
    # for i in range(3):
        if i != 0:
...
def get_preds_and_hidden_states(wiki_text_sentences, gpt2_model, gpt2_tokenizer, random_pos=True):
    ...
    for sentence in tqdm(wiki_text_sentences):
        if random_pos:
            ...
            print("Tokenized sentence length:", len(tokens_to_sent))
            print("Selected token position:", random_pos)
            ...
            if len(tokens_to_sent) > 0:
                if len(tokens_to_sent) == 1:
                    ...
                    # Uncomment the below line for the last token
                    # random_pos = len(tokens_to_sent) - 1

                    # Uncomment the below line for the middle token
                    random_pos = (len(tokens_to_sent)-1) // 2
...
parser.add_argument(
    "--max_sentences", default=100, type=int, help="max sentences to include in the data"
)
parser.add_argument(
...
# # trying simple small sample of sentence 
# sample_sentences = [
#     "The quick brown fox jumps over the lazy dog.",
#     "Machine learning is a subset of artificial intelligence.",
#     "The Eiffel Tower is located in Paris."
...
# trying SQuAD data load instead of wikitext: 
import json

with open('/Users/yafo/Library/Mobile Documents/com~apple~CloudDocs/IDC MLDS MSc 2021/1_Python/0_MLDS.Fall_TASHPAG/NLP_3523/ffn-values/train-v2.0.json', 'r') as file:
...
for article in squad_data['data']:
    for paragraph in article['paragraphs']:
        sentences.append(paragraph['context'])
...
df = get_examples_df(sentences[:args.max_sentences], model, tokenizer)
